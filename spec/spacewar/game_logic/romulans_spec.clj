(ns spacewar.game-logic.romulans-spec
  (:require [spacewar.game-logic.romulans :as r]
            ...
            [speclj.core :refer [with context describe it should should-be-nil should-not should=]]))
...
(describe "romulans behavior"
  ...
  (context "romulans state transitions"
  ...
  (it "transitions after the state duration at a second boundary"
    (with-redefs [rand (constantly 1)]
      (should (r/romulan-state-transition
                10
                (+ 1 (r/romulan-state-duration :invisible))
                :invisible))))

  (it "does not transition before the state duration"
    (with-redefs [rand (constantly 1)]
      (should-not (r/romulan-state-transition 10 0 :invisible))))

  (it "does not transition when the random roll fails"
    (with-redefs [rand (constantly 0)]
      (should-not (r/romulan-state-transition
                    10
                    (+ 1 (r/romulan-state-duration :invisible))
                    :invisible)))))
